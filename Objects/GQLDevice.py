...
class GQLDevice:
    ...
    def check_online(self, cfg: Config) -> None:
        ...
        if (datetime.now(timezone.utc) - time_delta) < self.last_updated:
            if (cfg.alerting.constant and self.last_ap != self.ap_mac_addr) or (
                datetime.now(timezone.utc) - self.last_alert > timedelta(hours=24)
            ):
                self.alert(cfg=cfg)
        return

    def update_info(self, save_info: dict) -> None:
        save_info.setdefault(self.uuid, {})["last_run"] = datetime.now(
            timezone.utc
        ).isoformat()
    ...
    def smtp_alert(self, cfg: Config) -> None:
        ...
        msg[
            "Subject"
        ] = f"Subject: Tracked Device Online - {self.uuid} on AP: {self.ap_name} at {self.last_updated.astimezone(cfg.timezone).strftime(fmt)}"
        msg.attach(
            MIMEText(
                f"""
            Tracked device:
        ...
        if cfg.alerting.smtp_use_tls:
            ...
            with smtplib.SMTP_SSL(
                cfg.alerting.smtp_host, cfg.alerting.smtp_port, context=context
            ) as server:
                server.login(
                    user=cfg.alerting.smtp_user, password=cfg.alerting.smtp_pass
                )
